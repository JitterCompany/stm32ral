...
pub mod POWER {
    ...
    /// Voltage switch sequence start. Start the timing critical section of the voltage switch sequence
    pub mod VSWITCH {
        /// Offset (2 bits)
...
pub mod STA {
    ...
    /// Inverted value of SDMMC_D0 line (Busy), sampled at the end of a CMD response and a second time 2 SDMMC_CK cycles after the CMD response. This bit is reset to not busy when the SDMMCD0 line changes from busy to not busy. This bit does not signal busy due to data transfer
    pub mod BUSYD0 {
        /// Offset (20 bits)
        pub const offset: u32 = 20;
        /// Mask (1 bit: 1 << 20)
    ...
    /// end of SDMMC_D0 Busy following a CMD response detected. This indicates only end of busy following a CMD response. This bit does not signal busy due to data transfer.
    pub mod BUSYD0END {
        /// Offset (21 bits)
...
/// Contains the acknowledgment timeout period, in SDMMC_CK bus clock periods. A counter loads the value from the SDMMC_ACKTIMER register, and starts decrementing when the data path state machine (DPSM) enters the Wait_Ack state. If the timer reaches 0 while the DPSM is in this states, the acknowledgment timeout status flag is set
pub mod ACKTIMER {

    /// Boot acknowledgement timeout period. Can only be written by firmware when CPSM is disabled (CPSMEN = 0)
    pub mod ACKTIME {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (25 bits: 0x1ffffff << 0)
...
/// Integrated DMA control register. Can only be written by firmware when DPSM is inactive (DPSMACT = 0)
pub mod IDMACTRLR {
...
/// The SDMMC_IDMABSIZER register contains the buffer size when in double buffer configuration
pub mod IDMABSIZER {
...
pub mod IDMABASE0R {
    ...
    /// Buffer 0 memory base address, must be word aligned (bit \[1:0\] are always 0 and read only). This register can be written by firmware when DPSM is inactive (DPSMACT = 0), and can dynamically be written by firmware when DPSM active (DPSMACT = 1) and memory buffer 0 is inactive (IDMABACT = 1).
    pub mod IDMABASE0 {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (32 bits: 0xffffffff << 0)
...
/// The SDMMC_IDMABASE0R register contains the double buffer configuration second buffer memory base address
pub mod IDMABASE1R {

    /// Buffer 1 memory base address, must be word aligned (bit \[1:0\] are always 0 and read only). This register can be written by firmware when DPSM is inactive (DPSMACT = 0), and can dynamically be written by firmware when DPSM active (DPSMACT = 1) and memory buffer 0 is inactive (IDMABACT = 1).
    pub mod IDMABASE1 {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (32 bits: 0xffffffff << 0)
...
/// FIFO word 0: ccan only be read/written as 32-bit word. there are 16 sequenctial register addresses FIFO0..FIFO15
pub mod FIFOR0 {

    /// Receive and transmit FIFO data This register can only be read or written by firmware when the DPSM is active (DPSMACT=1)
    pub mod FIFODATA {
        /// Offset (0 bits)
        pub const offset: u32 = 0;
        /// Mask (32 bits: 0xffffffff << 0)
...
/// FIFO word 1: ccan only be read/written as 32-bit word. there are 16 sequenctial register addresses FIFO0..FIFO15
pub mod FIFOR1 {
    pub use super::FIFOR0::FIFODATA;
}

/// FIFO word 2: ccan only be read/written as 32-bit word. there are 16 sequenctial register addresses FIFO0..FIFO15
pub mod FIFOR2 {
    pub use super::FIFOR0::FIFODATA;
}

/// FIFO word 3: ccan only be read/written as 32-bit word. there are 16 sequenctial register addresses FIFO0..FIFO15
pub mod FIFOR3 {
    pub use super::FIFOR0::FIFODATA;
}

/// FIFO word 4: ccan only be read/written as 32-bit word. there are 16 sequenctial register addresses FIFO0..FIFO15
pub mod FIFOR4 {
    pub use super::FIFOR0::FIFODATA;
}

/// FIFO word 5: ccan only be read/written as 32-bit word. there are 16 sequenctial register addresses FIFO0..FIFO15
pub mod FIFOR5 {
    pub use super::FIFOR0::FIFODATA;
}

/// FIFO word 6: ccan only be read/written as 32-bit word. there are 16 sequenctial register addresses FIFO0..FIFO15
pub mod FIFOR6 {
    pub use super::FIFOR0::FIFODATA;
}

/// FIFO word 7: ccan only be read/written as 32-bit word. there are 16 sequenctial register addresses FIFO0..FIFO15
pub mod FIFOR7 {
    pub use super::FIFOR0::FIFODATA;
}

/// FIFO word 8: ccan only be read/written as 32-bit word. there are 16 sequenctial register addresses FIFO0..FIFO15
pub mod FIFOR8 {
    pub use super::FIFOR0::FIFODATA;
}

/// FIFO word 9: ccan only be read/written as 32-bit word. there are 16 sequenctial register addresses FIFO0..FIFO15
pub mod FIFOR9 {
    pub use super::FIFOR0::FIFODATA;
}

/// FIFO word 10: ccan only be read/written as 32-bit word. there are 16 sequenctial register addresses FIFO0..FIFO15
pub mod FIFOR10 {
    pub use super::FIFOR0::FIFODATA;
}

/// FIFO word 11: ccan only be read/written as 32-bit word. there are 16 sequenctial register addresses FIFO0..FIFO15
pub mod FIFOR11 {
    pub use super::FIFOR0::FIFODATA;
}

/// FIFO word 12: ccan only be read/written as 32-bit word. there are 16 sequenctial register addresses FIFO0..FIFO15
pub mod FIFOR12 {
    pub use super::FIFOR0::FIFODATA;
}

/// FIFO word 13: ccan only be read/written as 32-bit word. there are 16 sequenctial register addresses FIFO0..FIFO15
pub mod FIFOR13 {
    pub use super::FIFOR0::FIFODATA;
}

/// FIFO word 14: ccan only be read/written as 32-bit word. there are 16 sequenctial register addresses FIFO0..FIFO15
pub mod FIFOR14 {
    pub use super::FIFOR0::FIFODATA;
}

/// FIFO word 15: ccan only be read/written as 32-bit word. there are 16 sequenctial register addresses FIFO0..FIFO15
pub mod FIFOR15 {
    pub use super::FIFOR0::FIFODATA;
}
#[repr(C)]
