...
/// I3C_CR and I3C_CR_ALTERNATE
/// I3C_CR: I3C message control register
/// I3C_CR_ALTERNATE: I3C message control register alternate
pub mod I3C_CR {
...
pub mod I3C_DEVR2 {
    pub use super::I3C_DEVR1::CRACK;
    pub use super::I3C_DEVR1::DA;
    pub use super::I3C_DEVR1::DIS;
    pub use super::I3C_DEVR1::IBIACK;
    pub use super::I3C_DEVR1::IBIDEN;
    pub use super::I3C_DEVR1::SUSP;
}
...
pub mod I3C_GETMXDSR {
    ...
    /// GETMXDS CCC format This field is written by software during bus initialization (i.e. I3C_CFGR.EN=0) and indicates how is returned the GETMXDS format 1 (without MaxRdTurn) and format 2 (with MaxRdTurn). This bit is used to return the 2-byte format 1 (MaxWr, MaxRd) or 5-byte format 2 (MaxWr, MaxRd, 3-byte MaxRdTurn) byte in response to the GETCAPS CCC. - 3-byte MaxRdTurn is returned with MSB=0, middle byte=0 and LSB=RDTURN\[7:0\]. - Max read turnaround time is less than 256 �s. - 3-byte MaxRdTurn is returned with MSB=0, middle byte=RDTURN\[7:0\] and LSB=0. - Max read turnaround time is between 256 �s and 65535 �s - 3-byte MaxRdTurn is returned with MSB=RDTURN\[7:0\], middle byte=0 and LSB=0. - Max read turnaround time is between 65535 �s and 16 s.
    pub mod FMT {
        /// Offset (8 bits)
        pub const offset: u32 = 8;
        /// Mask (2 bits: 0b11 << 8)
...
pub struct RegisterBlock {
    ...
    /// I3C_CR: I3C message control register
    /// I3C_CR_ALTERNATE: I3C message control register alternate
    pub I3C_CR: RWRegister<u32>,

    /// I3C configuration register
    pub I3C_CFGR: RWRegister<u32>,

    _reserved1: [u8; 8],

    /// I3C receive data byte register
    pub I3C_RDR: RORegister<u32>,

    /// I3C receive data word register
    pub I3C_RDWR: RORegister<u32>,

    /// I3C transmit data byte register
    pub I3C_TDR: WORegister<u32>,

    /// I3C transmit data word register
    pub I3C_TDWR: WORegister<u32>,

    /// I3C IBI payload data register
    pub I3C_IBIDR: RWRegister<u32>,

    /// I3C target transmit configuration register
    pub I3C_TGTTDR: RWRegister<u32>,

    _reserved2: [u8; 8],

    /// I3C status register
    pub I3C_SR: RORegister<u32>,

    /// I3C status error register
    pub I3C_SER: RORegister<u32>,

    _reserved3: [u8; 8],

    /// I3C received message register
    pub I3C_RMR: RORegister<u32>,

    _reserved4: [u8; 12],

    /// I3C event register
    pub I3C_EVR: RORegister<u32>,

    /// I3C interrupt enable register
    pub I3C_IER: RORegister<u32>,

    /// I3C clear event register
    pub I3C_CEVR: WORegister<u32>,

    _reserved5: [u8; 4],

    /// I3C own device characteristics register
    pub I3C_DEVR0: RWRegister<u32>,

    /// I3C device 1 characteristics register
    pub I3C_DEVR1: RWRegister<u32>,

    /// I3C device 2 characteristics register
    pub I3C_DEVR2: RWRegister<u32>,

    /// I3C device 3 characteristics register
    pub I3C_DEVR3: RWRegister<u32>,

    /// I3C device 4 characteristics register
    pub I3C_DEVR4: RWRegister<u32>,

    _reserved6: [u8; 28],

    /// I3C maximum read length register
    pub I3C_MAXRLR: RWRegister<u32>,

    /// I3C maximum write length register
    pub I3C_MAXWLR: RWRegister<u32>,

    _reserved7: [u8; 8],

    /// I3C timing register 0
    pub I3C_TIMINGR0: RWRegister<u32>,

    /// I3C timing register 1
    pub I3C_TIMINGR1: RWRegister<u32>,

    /// I3C timing register 2
    pub I3C_TIMINGR2: RWRegister<u32>,

    _reserved8: [u8; 20],

    /// I3C bus characteristics register
    pub I3C_BCR: RWRegister<u32>,

    /// I3C device characteristics register
    pub I3C_DCR: RWRegister<u32>,

    /// I3C get capability register
    pub I3C_GETCAPR: RWRegister<u32>,

    /// I3C controller-role capability register
    pub I3C_CRCAPR: RWRegister<u32>,

    /// I3C get capability register
    pub I3C_GETMXDSR: RWRegister<u32>,

    /// I3C extended provisioned ID register
    pub I3C_EPIDR: RWRegister<u32>,
}
pub struct ResetValues {
    ...
    pub I3C_MAXRLR: u32,
    pub I3C_MAXWLR: u32,
    pub I3C_TIMINGR0: u32,
    pub I3C_TIMINGR1: u32,
    pub I3C_TIMINGR2: u32,
    pub I3C_BCR: u32,
    pub I3C_DCR: u32,
    pub I3C_GETCAPR: u32,
    pub I3C_CRCAPR: u32,
    pub I3C_GETMXDSR: u32,
    pub I3C_EPIDR: u32,
}
#[cfg(not(feature = "nosync"))]
pub struct Instance {
...
/// Access functions for the I3C1 peripheral instance
pub mod I3C1 {
    ...
    /// Reset values for each field in I3C1
    pub const reset: ResetValues = ResetValues {
        ...
        I3C_TDR: 0x00000000,
        I3C_TDWR: 0x00000000,
        I3C_IBIDR: 0x00000000,
        I3C_TGTTDR: 0x00000000,
        I3C_SR: 0x00000000,
        I3C_SER: 0x00000000,
        I3C_RMR: 0x00000000,
        I3C_EVR: 0x00000003,
        I3C_IER: 0x00000000,
        I3C_CEVR: 0x00000000,
        I3C_DEVR0: 0x00000000,
        I3C_DEVR1: 0x00000000,
        I3C_DEVR2: 0x00000000,
        I3C_DEVR3: 0x00000000,
        I3C_DEVR4: 0x00000000,
        I3C_MAXRLR: 0x00000000,
        I3C_MAXWLR: 0x00000000,
        I3C_TIMINGR0: 0x00000000,
        I3C_TIMINGR1: 0x00000000,
        I3C_TIMINGR2: 0x00000000,
        I3C_BCR: 0x00000000,
        I3C_DCR: 0x00000000,
        I3C_GETCAPR: 0x00000000,
        I3C_CRCAPR: 0x00000000,
        I3C_GETMXDSR: 0x00000000,
        I3C_EPIDR: 0x02080000,
    };
    ...
    #[no_mangle]
    static mut I3C1_TAKEN: bool = false;
    ...
    /// Release exclusive access to I3C1
    ///
    ...
    pub fn release(inst: Instance) {
        external_cortex_m::interrupt::free(|_| unsafe {
            if I3C1_TAKEN && inst.addr == INSTANCE.addr {
                I3C1_TAKEN = false;
            } else {
...
pub const I3C1: *const RegisterBlock = 0x40005c00 as *const _;

/// Access functions for the I3C2 peripheral instance
pub mod I3C2 {
    use super::ResetValues;
    ...
    /// Reset values for each field in I3C2
    pub const reset: ResetValues = ResetValues {
        I3C_CR: 0x00000000,
        I3C_CFGR: 0x00000000,
        I3C_RDR: 0x00000000,
    ...
    #[allow(private_no_mangle_statics)]
    #[no_mangle]
    static mut I3C2_TAKEN: bool = false;
    ...
    /// Release exclusive access to I3C2
    ///
    /// This function allows you to return an `Instance` so that it
    /// is available to `take()` again. This function will panic if
    /// you return a different `Instance` or if this instance is not
    ...
    pub fn release(inst: Instance) {
        external_cortex_m::interrupt::free(|_| unsafe {
            if I3C2_TAKEN && inst.addr == INSTANCE.addr {
                I3C2_TAKEN = false;
            } else {
                panic!("Released a peripheral which was not taken");
            }
...
/// This constant is provided for ease of use in unsafe code: you can
/// simply call for example `write_reg!(gpio, GPIOA, ODR, 1);`.
pub const I3C2: *const RegisterBlock = 0x44003000 as *const _;
